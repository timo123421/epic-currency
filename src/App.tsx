        ...
        <table className="w-full text-left text-sm whitespace-nowrap">
          ...
          <tbody className="divide-y divide-[#ffffff0a]">
            {history.map((tx, i) => {
              ...
              if (
                tx.sender === "CYBERHEAVEN_UNIVERSITY_TREASURY" &&
                !isSender
              ) {
                type = "Earned";
              ...
              if (
                tx.recipient === "CYBERHEAVEN_UNIVERSITY_TREASURY" &&
                isSender
              ) {
                type = "Spent";
...
export default function App() {
      ...
      try {
        ...
        fetch("/api/auth/restore", {
          ...
          headers: {
            ...
            Authorization: `Bearer ${savedWallet.token}`,
          },
          ...
          if (res.ok) {
            ...
            addMessage(
              "system",
              "Quantum session tether re-established from local storage.",
            );
            fetchUsers();
          } else {
            localStorage.removeItem("cyberheaven_wallet");
            ...
            if (tx.status === "confirmed") {
              if (!knownTxIds.has(tx.id)) {
                if (
                  ...
                  addMessage(
                    "system",
                    `INCOMING TRANSFER: You received ${tx.amount} CHT.`,
                  );
                  hasNewReceives = true;
                }
      ...
      if (currentWalletAddress) {
        ...
        try {
          const token = JSON.parse(
            localStorage.getItem("cyberheaven_wallet")!,
          ).token;
    ...
    ...apiChatMessages
      .filter((m) => {
        ...
        if (activeChannel.startsWith("dm-")) {
          ...
          return (
            (m.senderAddress === partner &&
              m.recipientAddress === wallet?.address) ||
            (m.recipientAddress === partner &&
              m.senderAddress === wallet?.address)
          );
        }
        return false;
      })
  ...
  const dmPeers = Array.from(
    new Set(
      apiChatMessages
        .filter((m: any) => m.recipientAddress !== "global")
        .flatMap((m: any) => [m.senderAddress, m.recipientAddress])
        .filter((a: any) => a !== wallet?.address),
    ),
  );
  ...
  const handleGateSubmit = async (e: React.FormEvent) => {
    ...
    if (gateMode === "login") {
      try {
        ...
        addMessage(
          "archangel",
          `Authentication successful. Welcome back, **${data.profile.username}**.`,
        );
      } catch (e: any) {
      ...
      try {
        ...
        addMessage(
          "archangel",
          `Identity forged. Welcome, **${data.profile.username}**. You carry the rank of **${data.profile.role}**.\n\nYour address is \`${data.address}\` secured via **${data.algorithm}**.`,
        );
      } catch (e: any) {
    ...
    setTimeout(async () => {
      try {
        if (cmd === "/connect") {
          addMessage(
            "system",
            "Connection is now managed through the Cyberheaven Gate UI upon entry.",
          );
        } else if (cmd === "/teach") {
          if (!wallet) throw new Error("You must `/connect` first.");
          if (wallet.profile.role === "Initiate")
            throw new Error(
              "Initiates lack the wisdom to `/teach`. Enroll in courses to ascend to Scholar or Archangel.",
            );
          const course = args.join(" ");
          if (!course)
            throw new Error(
              "Provide a subject to teach, e.g. `/teach Quantum Fields 101`",
            );
          ...
          if (!course)
            throw new Error(
              "Provide a subject to learn, e.g. `/learn Advanced Lattice Cryptography`",
            );
          ...
          addMessage(
            "archangel",
            `Your current essence is **${wallet.balance} CHT**.\nAddress: \`${wallet.address}\``,
          );
        } else if (cmd === "/send") {
          if (!wallet) throw new Error("You must `/connect` first.");
          ...
          if (isNaN(amount) || !recipient)
            throw new Error("Correct format: `/send <amount> <address>`");
          ...
          const res = await fetch("/api/crypto/sign", {
            ...
            body: JSON.stringify({
              ...
              recipient,
              amount,
              publicKey: wallet.publicKeyFull,
          ...
          addMessage(
            "system",
            "Quantum-Safe signature constructed. Appended to Mempool.",
          );
          addMessage(
            "archangel",
            `Transaction signed using **${wallet.algorithm}** (Signature size: ${data.signatureSizeBytes} bytes).\n\nAuto-triggering Network consensus...`,
          );
          ...
          try {
            ...
            if (l2Res.ok) {
              addMessage(
                "system",
                "Compressing & Submitting ZK-Proof to Validators...",
              );
              ...
              if (l3Res.ok) {
                addMessage(
                  "archangel",
                  `✅ Consensus reached. ${amount} CHT has been successfully delivered to the recipient.\n\nTX ID: \`${data.txId}\``,
                );
                await updateBalance(wallet.address);
              }
            }
          ...
          addMessage(
            "system",
            "Triggering Layer 2 ZKP Compression over current Mempool...",
          );
          ...
          addMessage(
            "archangel",
            `Compressed ${l2Data.txCount} bloated lattice transaction(s) down to a **${l2Data.compressedBytes} byte** ZK-SNARK.`,
          );

          addMessage(
            "system",
            "Submitting ZK-Proof to Layer 3 Cyberheaven Validators...",
          );
          const l3Res = await fetch("/api/network/consensus", {
            method: "POST",
            headers: {
          ...
          addMessage(
            "archangel",
            `Consensus reached via **${l3Data.mechanism}**. Block **${l3Data.blockHeight}** finalized in ${l3Data.finalityTimeMs}ms with hash \`${l3Data.blockHash}\`. ${l3Data.processedTransactions} transactions applied to ledger.`,
          );
          await updateBalance(wallet.address);
        } else if (cmd === "/history") {
          if (!wallet) throw new Error("You must `/connect` first.");
          ...
          addMessage(
            "archangel",
            `Unknown command. Type \`/help\` for the sacred rites.`,
          );
        } else {
          ...
          if (token) {
            const mRes = await fetch("/api/chat/messages", {
              headers: { Authorization: `Bearer ${token}` },
            });
            if (mRes.ok) {
  ...
  return (
    <div className="h-[100dvh] w-full bg-[#020205] flex items-center justify-center p-2 sm:p-4 md:p-8 overflow-hidden">
      <div className="flex bg-[#06080E] h-full min-h-0 w-full max-w-[1600px] max-h-[1000px] rounded-lg sm:rounded-xl border border-[#ffffff0a] shadow-[0_0_80px_rgba(0,0,0,0.8)] text-slate-300 font-sans selection:bg-cyan-900/50 relative overflow-hidden">
        {/* Background Ambience */}
        ...
        <div
          className={cn(
            "w-64 shrink-0 bg-[#090b14] border-r border-[#ffffff0a] flex flex-col z-20 absolute md:relative h-full min-h-0 transition-transform duration-300 shadow-2xl md:shadow-none",
            isMobileMenuOpen
              ? "translate-x-16"
              : "-translate-x-[120%] md:translate-x-0 !ml-0",
          )}
        >
          <div className="h-16 flex items-center justify-between px-4 border-b border-[#ffffff0a] shadow-sm bg-[#090b14] sticky top-0 z-10 shrink-0">
            <span className="gold-text-glow font-serif text-[#d4af37] font-bold text-lg tracking-wide">
              Cyberheaven Univ.
            </span>
            <button
            ...
            <div>
              ...
              <div className="space-y-0.5">
                <ChannelItem
                  icon={<Hash />}
                  label="general-chat"
                  active={activeChannel === "chat"}
                  onClick={() => setActiveChannel("chat")}
                />
                <ChannelItem
                  icon={<BookOpen />}
                  label="courses-board"
                  active={activeChannel === "courses"}
                  onClick={() => setActiveChannel("courses")}
                />
                <ChannelItem
                  icon={<Network />}
                  label="quantum-ledger"
                  active={activeChannel === "ledger"}
                  onClick={() => {
                    setActiveChannel("chat");
              ...
              <div>
                ...
                <div className="space-y-0.5">
                  {dmPeers.map((peerAddr: string) => {
                    const u = onlineUsers.find(
                      (ou) => ou.address === peerAddr,
                    ) || { username: peerAddr.substring(0, 8) };
                    return (
                      <ChannelItem
                        key={peerAddr}
                        icon={<MessageSquare />}
                        label={u.username}
                        active={activeChannel === `dm-${peerAddr}`}
                        onClick={() => setActiveChannel(`dm-${peerAddr}`)}
                      />
                    );
                  })}
                </div>
              ...
              <div className="space-y-0.5">
                {wallet?.profile.role === "Archangel" && (
                  <ChannelItem
                    ...
                    active={activeChannel === "vault"}
                    onClick={() => setActiveChannel("vault")}
                  />
                )}
                <ChannelItem
                  icon={<Shield />}
                  label="pqc-validators"
                  onClick={() => {
                    setActiveChannel("chat");
                    handleCommand(undefined, "/balance");
                  }}
                />
                <ChannelItem
                  icon={<Cpu />}
                  label="l2-zkp-compression"
                  onClick={() => {
                    setActiveChannel("chat");
                    handleCommand(undefined, "/network");
                  }}
                />
                <ChannelItem
                  icon={<Server />}
                  label="l3-dag-consensus"
                  onClick={() => {
                    setActiveChannel("chat");
                    handleCommand(undefined, "/network");
                  }}
        ...
        <div className="flex-1 flex flex-col min-w-0 min-h-0 z-10 relative">
          <header className="h-16 border-b border-[#ffffff0a] flex items-center px-4 md:px-6 gap-3 md:gap-4 bg-[#06080E] z-20 shrink-0 relative">
            <button
              className="md:hidden text-slate-400 hover:text-white"
            ...
            <div className="flex-1 min-w-0">
              <div className="font-bold text-white tracking-wide truncate">
                ...
                {activeChannel.startsWith("dm-") &&
                  `direct-message-${(onlineUsers.find((u) => u.address === activeChannel.replace("dm-", "")) || {}).username || activeChannel.replace("dm-", "").substring(0, 6)}`}
              </div>
              <div className="text-[10px] text-slate-400 font-mono uppercase tracking-widest truncate">
                ...
                {activeChannel === "courses" &&
                  "Enroll in syllabuses. Expand your node."}
              ...
              <>
                        ...
                        {m.sender !== "system" && (
                          <div
                            className={cn(
                              ...
                              m.sender === "archangel"
                                ? "bg-black border-[#d4af37]/50 gold-glow"
                                : m.sender === "peer"
                                  ? "bg-[#0a0f1a] border-purple-500/50"
                        ...
                        <div className="flex-1 min-w-0">
                          {m.sender !== "system" && (
                            <div className="flex items-baseline gap-2 mb-1">
                              ...
                              {m.sender === "archangel" && (
                                <span className="text-[9px] bg-[#d4af37]/20 text-[#d4af37] px-1.5 py-0.5 rounded font-bold uppercase tracking-wider">
                                  Verified App
                                </span>
                              )}
                          ...
                          <div
                            className={cn(
                              ...
                              m.sender === "system"
                                ? "text-cyan-400 font-mono text-[10px] uppercase tracking-widest bg-cyan-900/10 py-1 px-3 border-l-2 border-cyan-400"
                                : "text-slate-200",
                            )}
                          >
                            {m.sender === "user" ? (
                ...
                <div className="p-4 md:p-6 pt-0 mt-auto">
                  <form
                    ...
                    className="relative flex items-center bg-[#0a0f1a] border border-[#ffffff1a] rounded-xl overflow-hidden focus-within:border-cyan-500/50 transition-colors gold-glow"
                  >
                    ...
                    <input
                      ...
                      placeholder={
                        activeChannel === "chat"
                          ? wallet?.profile.role === "Initiate"
                            ? "Message #general-chat (/study, /balance, /help)..."
                            : "Message #general-chat (/teach, /balance, /help)..."
                          : "Send Direct Message..."
                      }
                    ...
                    <button
                      ...
                      className="absolute right-2 p-2 bg-black/50 hover:bg-[#d4af37]/20 text-[#d4af37] rounded-lg transition-colors disabled:opacity-50"
                    >
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {catalog.map((course) => (
                    <div
                      key={course.title}
                      className="bg-black/40 border border-[#ffffff0a] rounded-xl p-6 gold-glow flex flex-col hover:border-[#d4af37]/30 transition-colors duration-500"
                    >
                      ...
                      <p className="text-slate-400 text-sm mb-6 flex-1">
                        Enroll in this syllabus to enhance your internal
                        knowledge node in the collective.
                      </p>
        ...
        <div
          className={cn(
            "shrink-0 bg-[#090b14] border-l border-[#ffffff0a] z-50 flex flex-col transition-all duration-300 absolute md:relative right-0 h-full min-h-0 shadow-2xl md:shadow-none",
            isMembersOpen ? "w-72" : "w-0 md:w-72 overflow-hidden",
          )}
        >
          <div className="h-16 shrink-0 flex items-center px-4 border-b border-[#ffffff0a] font-bold text-xs uppercase tracking-widest text-[#d4af37] justify-between bg-[#090b14] z-10 sticky top-0">
            ...
            <div className="bg-black/40 border border-[#ffffff0a] rounded-xl p-4 flex flex-col items-center text-center relative overflow-hidden">
              ...
              {wallet && (
                <div className="mt-4 w-full bg-[#030408] border border-cyan-900/50 rounded-lg p-3">
                  ...
                  <div className="text-2xl font-serif text-[#d4af37] font-bold">
                    {wallet.balance} <span className="text-xs">CHT</span>
                  </div>
                </div>
              )}
            </div>
            ...
            {wallet ? (
              <div className="space-y-4">
                <div className="text-xs font-bold uppercase tracking-widest text-slate-500 flex flex-col gap-2 border-b border-[#ffffff0a] pb-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      className="text-[9px] text-cyan-400 cursor-pointer hover:bg-cyan-900/30 border border-cyan-400/30 px-1.5 py-0.5 rounded bg-cyan-900/10 transition-colors"
                      onClick={() => {
                        handleCommand(undefined, "/history");
                        if (isMembersOpen) setIsMembersOpen(false);
                      }}
                    ...
                    <button
                      className="text-[9px] text-red-400 cursor-pointer hover:bg-red-900/30 border border-red-400/30 px-1.5 py-0.5 rounded bg-red-900/10 transition-colors"
                      onClick={() => {
                        localStorage.removeItem("cyberheaven_wallet");
                        setWallet(null);
                        setShowGate(true);
                        ...
                        addMessage(
                          "system",
                          "Quantum session severed. Identity removed from local node.",
                        );
                      }}
                    >
                      Sever Link 🔌
                ...
                <div className="space-y-1">
                  ...
                  <div className="text-sm text-white font-mono bg-black/50 border border-[#ffffff0a] p-2 rounded">
                    {wallet.algorithm}
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="text-[10px] text-cyan-400 font-mono bg-cyan-900/10 border border-cyan-900/30 p-2 rounded break-all">
                    {wallet.address}
                  </div>
                </div>

                <div className="mt-8">
                  <div className="text-xs font-bold uppercase tracking-widest text-slate-500 border-b border-[#ffffff0a] pb-2 mb-3">
              ...
              <div className="flex flex-col items-center justify-center text-center p-6 border border-dashed border-[#ffffff1a] rounded-xl">
                ...
                <div className="text-[10px] text-slate-500 font-mono">
                  Use the Gate to establish quantum link.
                </div>
              </div>
            )}
          </div>
        ...
        {showGate && (
          <div className="absolute inset-0 z-[100] bg-[#06080E]/95 backdrop-blur-md overflow-y-auto flex items-center justify-center p-4 sm:p-8">
            <div className="max-w-md w-full bg-black/90 border border-[#d4af37]/40 rounded-2xl p-8 shadow-2xl gold-glow relative overflow-hidden my-8">
              ...
              <h2 className="text-center text-2xl font-serif font-bold text-white mb-2">
                Cyberheaven Gate
              </h2>
              <p className="text-center text-xs text-slate-400 font-mono mb-8">
                Establish quantum-resistant ephemeral identity.
              </p>
              ...
              <div className="flex gap-4 mb-6">
                <button
                  ...
                  className={cn(
                    "flex-1 pb-2 border-b-2 font-bold text-sm tracking-widest uppercase transition-colors",
                    gateMode === "login"
              ...
              <form onSubmit={handleGateSubmit} className="space-y-5">
                ...
                {gateMode === "register" && (
                  <>
                    <div>
                      ...
                      >
                        <option value="Initiate">
                          Initiate (Student - Learns)
                        </option>
                        <option value="Scholar">
                          Scholar (Teacher - Earns)
                        </option>
                        <option value="Archangel">
                          Archangel (Admin - Vault Access)
                        </option>
                      </select>
                    </div>
                    ...
                    <div>
                      ...
                      >
                        ...
                        <option value="ML-DSA-65">
                          ML-DSA-65 (High Security)
                        </option>
                ...
                <button
                  type="submit"
                  className="w-full bg-[#d4af37] text-black font-bold py-3 rounded-lg hover:bg-white transition-colors mt-6 font-serif shadow-[0_0_20px_rgba(212,175,55,0.4)] cursor-pointer"
                >
                  {gateMode === "login"
                    ? "Authenticate"
